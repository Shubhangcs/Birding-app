        ...
        actions: [
          IconButton(
            onPressed: () {
              showSearch(
                context: context,
                delegate: CustomSearch(searchBloc: searchBloc),
              );
            },
            icon: const Icon(
      ...
      builder: (context, state) {
        return ListView.builder(
          itemBuilder: (context, index) {
            return ListTile(
              title:(state is HomePageSearchDataFetchState)?Text(state.data[index]):null,
              trailing: const Icon(Icons.arrow_outward_rounded),
            );
          },
          itemCount:(state is HomePageSearchDataFetchState)?state.data.length:0 ,
        );
      },
    );
  }
